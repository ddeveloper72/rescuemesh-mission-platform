...
if [ "$MISSION_COUNT" = "0" ]; then
  ...
  python manage.py seed_usecases || echo "  WARNING: Use case seeding skipped or failed"

  # Create demo missions with fixed UUIDs (required for frontend)
  echo "  - Creating demo missions..."
  python manage.py seed_demo_missions || echo "  WARNING: Demo mission creation skipped or failed"

  # Load digital twin terrain data (if available)
  if [ -f /data/processed/migovec_sample.json ]; then
    echo "  - Loading digital twin sites..."
    python manage.py seed_digital_twins || echo "  WARNING: Digital twin seeding skipped or failed"
  fi
  ...
  if python manage.py seed_mission_scenarios --all; then
    ...
    echo "    WARNING: Scenario seeding failed - simulations may not work"
  ...
  python manage.py seed_media_artifacts || echo "  WARNING: Media artifact seeding skipped or failed"
...
python manage.py reset_stale_simulations --reset-all || echo "  WARNING: Simulation reset skipped or failed"
